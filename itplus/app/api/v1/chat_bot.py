...
def send_message(
    ...
    svc = ConversationService(db)
    try:
        conversation, response, is_finished = svc.send_message(
...
def finish_conversation(
    ...
    svc = ConversationService(db)
    try:
        conversation, response, is_finished = svc.finish_conversation_manually(conversation_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
def get_conversation(
    ...
    return ConversationDetailResponse(
        ...
        messages=[MessageResponse.model_validate(m) for m in visible_messages],
    )
