...
import { getItplusHealth, listDocuments } from '../../api/itplus'

export default function ItplusDashboardPage() {
  const navigate = useNavigate()
  const [health, setHealth] = useState<Record<string, unknown> | null>(null)
  const [docStats, setDocStats] = useState({ total: 0, ready: 0 })

  useEffect(() => {
    getItplusHealth().then(setHealth).catch(() => setHealth(null))
    listDocuments()
      .then((docs) => {
        setDocStats({
          total: docs.length,
